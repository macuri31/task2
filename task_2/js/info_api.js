...
//    let responce = await fetch("../../tmp/g06u28.shop.json")
    let responce = await fetch("./data/info.json")
    ...
    for (key in content) {
        ...
        <li style="min-height: 60vh" class="d-flex flex-column m-1 p-1 border bg-body>
        <img style="object-fit:cover;
	float:left; 
    width: 300px;
	margin-top: 70px;
	margin-bottom: 10px;" class="img-info" src=${content[key].img}>
        <h style="" class="line-info">${content[key].title}</h>
        <p class="text-inf"> ${content[key].description}</p>
